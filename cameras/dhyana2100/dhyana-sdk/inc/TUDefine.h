...
typedef enum
{
	...
	TUIDI_CONNECTSTATUS         = 0x0F,             // the Camera Whether connection
	TUIDI_CAMERA_LOG            = 0x10,             // the Camera Log Message
	TUIDI_CAMERA_PAYLOADSIZE    = 0x11,             // the Camera PayloadSize
    TUIDI_ENDINFO               = 0x12,             // the string id end
}TUCAM_IDINFO;
...
typedef enum
{
	...
	TUIDC_BUFFERHEIGHT          = 0x32,             // id capability buffer height
	TUIDC_VISIBILITY            = 0x33,             // id capability visibility
    TUIDC_ENDCAPABILITY         = 0x34,             // id capability end 
}TUCAM_IDCAPA;
...
{   
    ...
    TUIDCR_END                  = 0x02,             // id calculate roi end
}TUCAM_IDCROI;
...
typedef enum
{
	...
	TU_CAMERABASE_XML             = 0x01,  // [in] the device of the camera base xml 
	TU_CAMERALINK_XML             = 0x02,  // [in] the device of the camera link xml 
} TUXML_DEVICE;
...
//  typedef struct call back buf info
typedef struct _tagTUCAM_CB_BUFHEADER
{
	BOOL   bFilled;         // [out] The buf filled

	UINT32 uiWidth;         // [out] The buf width
	UINT32 uiHeight;        // [out] The buf height
	UINT32 uiXOffset;       // [out] The buf x offset
	UINT32 uiYOffset;       // [out] The buf y offset
	UINT32 uiXPadding;      // [out] The buf x padding
	UINT32 uiYPadding;      // [out] The buf y padding
	UINT32 uiImgOffset;     // [out] The buf offset
	UINT32 uiPixelFormat;   // [out] The buf pixel format
	UINT32 uiImgSize;       // [out] The buf size
	UINT32 uiIndex;         // [out] The image index number

	PUCHAR pImgData;        // [out] Pointer to the image data
} TUCAM_CB_BUFHEADER, *PTUCAM_CB_BUFHEADER;
